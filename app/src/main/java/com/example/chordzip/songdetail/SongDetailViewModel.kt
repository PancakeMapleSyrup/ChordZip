...
class SongDetailViewModel @Inject constructor(
    private val songDao: SongDao, // DAO 주입
    savedStateHandle: SavedStateHandle // 네비게이션으로 넘어온 songId를 받기 위함
) : ViewModel() {
    ...
    // 코드 추가 (특정 파트에)
    fun addChord(partId: String, chordName: String, positions: List<Int>, startFret: Int) {
        val currentSong = _uiState.value.song ?: return
        ...
        val updatedParts = currentSong.parts.map { part ->
            if (part.id == partId) {
                ...
                part.copy(chords = part.chords + newChord)
            } else part
        }
        val updatedSong = currentSong.copy(parts = updatedParts)
    ...
    // 코드 수정 기능
    fun updateChord(partId: String, chordId: String, newName: String, newPositions: List<Int>, newStartFret: Int) {
        val currentSong = _uiState.value.song ?: return

        val updatedParts = currentSong.parts.map { part ->
            if (part.id == partId) {
                // 해당 파트 안에서, 수정하려는 코드(chordId)를 찾아서 교체
                val updatedChords = part.chords.map { chord ->
                    if (chord.id == chordId) {
                        chord.copy(
                            name = newName,
                ...
                part.copy(chords = updatedChords)
            } else {
                part
            }
        }
